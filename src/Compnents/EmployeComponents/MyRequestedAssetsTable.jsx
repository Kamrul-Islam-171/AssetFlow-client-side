...
const MyRequestedAssetsTable = ({ assets, refetch }) => {
                    ...
                    <tbody>

                        {
                            assets?.map((item, idx) => <tr key={item._id}>
                                ...
                                <td>{item.Request}</td>
                                <td>
                                    {item.Request === 'pending' && <button className='btn bg-primary-color text-white border-0 hover:text-primary-color hover:bg-white'>Cancel</button>}
